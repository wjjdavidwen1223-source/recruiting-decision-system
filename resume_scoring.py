...
def score_candidate_against_profile(row, profile_key: str):
    ...
    return score, max_score, matched_signals, missing_signals, signal_map
...
def decision_from_profile_score(score, profile_key, signal_map):
    ...
    minimum_must_have_needed = max(1, len(must_have_signals) - 1)

    if must_have_hits < minimum_must_have_needed:
        return "Reject"
...
def build_reason_and_improvement(profile_label, matched_signals, missing_signals, decision):
    strong = [prettify_signal(s) for s in matched_signals[:5]]
    weak = [prettify_signal(s) for s in missing_signals[:5]]

    if decision == "Interview":
        reason = (
            f"Strong fit for {profile_label}. Clear evidence across "
    ...
    if decision == "Hold":
        reason = (
            f"Partial fit for {profile_label}. Candidate shows strength in "
    ...
    reason = (
        f"Currently below target match for {profile_label}. Missing stronger evidence in "
...
def run_screening(df, profile_key="generic_retail_banker"):
    ...
    for _, row in result.iterrows():
        score, max_score, matched_signals, missing_signals, signal_map = score_candidate_against_profile(
            row, profile_key
        )

        decision = decision_from_profile_score(score, profile_key, signal_map)
        reason, improvement = build_reason_and_improvement(
            role_label, matched_signals, missing_signals, decision
        )
